...
const themes = [
  { name: "Cyberpunk Neon", icon: "🌆", c: C },
  { name: "Matrix Green", icon: "🌿", c: G },
  { name: "Crimson Heist", icon: "🩸", c: R },
  { name: "Gold Elite", icon: "👑", c: Y },
  { name: "Ice Blue", icon: "❄️", c: "#88CCFF" },
  { name: "Dark Stealth", icon: "🥷", c: "#666688" },
  { name: "Light Clean", icon: "☁️", c: "#AABBCC" },
  { name: "Student Mode", icon: "🎓", c: "#8866FF" },
...
function ChartCanvas({ type, color = C }) {
  ...
  useEffect(() => {
    ...
    if (type === "overview") {
      ...
      ctx.strokeStyle = R + "40"; ctx.setLineDash([4,4]); ctx.beginPath(); ctx.moveTo(80,50); ctx.lineTo(420,50); ctx.stroke(); // OR-H
      ctx.strokeStyle = G + "40"; ctx.beginPath(); ctx.moveTo(80,130); ctx.lineTo(420,130); ctx.stroke(); // OR-L
      ctx.setLineDash([]);
      ...
      ctx.strokeStyle = M + "25"; ctx.strokeRect(80, 30, 340, 140);
      ...
      ctx.fillStyle = "#ffffffaa"; ctx.fillText("PD MIDLINE", 10, 88);
      // NY Open
      ctx.strokeStyle = Y + "40"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(80,100); ctx.lineTo(420,100); ctx.stroke();
      ctx.fillStyle = Y; ctx.fillText("NY OPEN", 10, 103);
      // Candles
      const prices = [95,92,88,85,82,78,80,85,90,95,98,102,105,108,112,110,106,102,98,95];
      prices.forEach((p,i) => {
        const x = 90 + i * 17;
        const o = CH - p - 5 + (Math.random()-0.5)*4;
        const cl = CH - p + (Math.random()-0.5)*4;
        ctx.fillStyle = cl < o ? G : R;
        ctx.fillRect(x-2, Math.min(o,cl), 4, Math.abs(cl-o)||1);
      });
      ctx.fillStyle = C; ctx.font = "bold 10px monospace"; ctx.fillText("4 LAYERS = COMPLETE CONTEXT", CW/2 - 90, CH - 8);
    } else if (type === "or") {
      ...
      ctx.fillStyle = Y; ctx.font = "bold 9px monospace"; ctx.fillText("15 MIN BUILD → LOCK → EXTENDS ALL DAY", 60, CH-10);
    } else if (type === "band") {
      // Tradable band with open anchor
      ctx.fillStyle = M + "08"; ctx.fillRect(50, 30, 370, 140);
      ctx.strokeStyle = M + "30"; ctx.strokeRect(50, 30, 370, 140);
      // Open anchor
      ...
      // Band edges
      ctx.fillStyle = M; ctx.font = "8px monospace"; ctx.fillText("BAND TOP = open + (PDR × 0.30)", 60, 28);
      ctx.fillText("BAND BOT = open - (PDR × 0.30)", 60, CH-20);
      ctx.fillStyle = M; ctx.fillText("BAND MID = open (bias line)", 220, 96);
      // Arrow showing 60% of PDR
      ctx.strokeStyle = "#ffffff20"; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(30,30); ctx.lineTo(30,170); ctx.stroke();
      ctx.fillStyle = "#ffffff30"; ctx.font = "9px monospace"; ctx.fillText("60%", 15, 100); ctx.fillText("PDR", 15, 112);
    }
  }, [type, color]);
...
export default function AuraMapGuide() {
  ...
  const renderOverview = () => (
    <div>
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        ...
        <p style={{ color: D, fontSize: 13, marginTop: 8, lineHeight: 1.5 }}>Visual Intelligence Overlay<br/>Opening Range · Tradable Band · PD Mid · Session Opens</p>
      ...
      <Bx color={G} style={{ marginTop: 16 }}>
        <Glow color={G} size="0.85rem">📡 WHAT IS AURA MAP?</Glow>
        <p style={{ color: "#ccc", fontSize: 13, lineHeight: 1.7, marginTop: 8 }}>AURA MAP isn't a signal system — it's a <strong style={{color:C}}>context system</strong>. It draws every key level you need on your chart automatically: Opening Range, Tradable Band, Previous Day Midline, and Session Opens. Layer it under any AURA SZN weapon and suddenly every signal has context: <strong style={{color:Y}}>"Is this long in discount? Inside the Opening Range? Above or below the open?"</strong></p>
      </Bx>
      <Bx color={Y} style={{ marginTop: 12 }}>
        <Glow color={Y} size="0.85rem">📊 THE 4 LAYERS</Glow>
          ...
          {[
            ...
            <div key={i} style={{ padding: "10px 12px", borderRadius: 8, background: `${l.c}06`, border: `1px solid ${l.c}18` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 12, color: l.c, fontWeight: "bold" }}>{l.name}</span>
              </div>
              <div style={{ fontSize: 10, color: "#888", marginTop: 4 }}>{l.d}</div>
            </div>
          ))}
        </div>
      </Bx>
      <Bx color={C} style={{ marginTop: 12 }}>
        ...
        <p style={{ color: "#999", fontSize: 13, lineHeight: 1.6, marginTop: 6 }}><strong style={{color:C}}>Google Maps</strong> for your chart. You COULD drive without GPS, but why? AURA MAP marks the roads (levels), intersections (opens), and neighborhoods (premium vs discount) so you always know where you are.</p>
      </Bx>
    </div>
  );
  ...
  const renderOR = () => (
    <div>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <Glow color={C} size="1.5rem">📊 OPENING RANGE</Glow>
        <p style={{ color: D, fontSize: 12, marginTop: 4 }}>First 15 minutes of NY. The most important zone of the day.</p>
      </div>
      <ChartCanvas type="or" color={C} />
      <Bx color={C} style={{ marginTop: 16 }}>
        <Glow color={C} size="0.85rem">WHAT IT DOES</Glow>
        <p style={{ color: "#ccc", fontSize: 13, lineHeight: 1.7, marginTop: 8 }}>At 9:30 AM, the box starts building. For 15 minutes, it tracks the high and low. At 9:45, it <strong style={{color:Y}}>locks</strong> and draws OR-High (red), OR-Low (green), and OR-Mid (dotted) lines that extend through the entire day.</p>
      </Bx>
      <Bx color={G} style={{ marginTop: 10 }}>
        <Glow color={G} size="0.85rem">🎯 HOW TO TRADE IT</Glow>
        <div style={{ marginTop: 8, display: "grid", gap: 8 }}>
          {[
            ...
            { play: "OR as Context", d: "If your signal fires INSIDE the OR, be cautious — it's a compressed zone. If it fires OUTSIDE, the move has more room to run.", c: C },
          ].map((p, i) => (
            <div key={i} style={{ padding: "10px 12px", borderRadius: 8, background: `${p.c}06`, borderLeft: `3px solid ${p.c}40` }}>
              <div style={{ fontSize: 12, color: p.c, fontWeight: "bold" }}>{p.play}</div>
              <div style={{ fontSize: 11, color: "#aaa", marginTop: 4, lineHeight: 1.5 }}>{p.d}</div>
            </div>
          ))}
        </div>
      </Bx>
      <Bx color={M} style={{ marginTop: 10 }}>
        ...
        <p style={{ color: "#999", fontSize: 13, lineHeight: 1.6, marginTop: 6 }}>The coin toss at a football game. The first 15 minutes decide who has momentum. The OR is the field — the lines tell you where the end zones are.</p>
      </Bx>
    </div>
  );
  ...
  const renderBand = () => (
    <div>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <Glow color={M} size="1.5rem">🎯 TRADABLE BAND</Glow>
        <p style={{ color: D, fontSize: 12, marginTop: 4 }}>How far can price ACTUALLY go today?</p>
      </div>
      <ChartCanvas type="band" color={M} />
      <Bx color={M} style={{ marginTop: 16 }}>
        <Glow color={M} size="0.85rem">WHAT IT DOES</Glow>
        <p style={{ color: "#ccc", fontSize: 13, lineHeight: 1.7, marginTop: 8 }}>Takes <strong style={{color:Y}}>yesterday's range</strong> (high – low) × 0.60 = the expected move for today. Centers it on the 9:30 open price. Half above, half below. This box is where price will spend MOST of its time.</p>
      </Bx>
      <Bx color={G} style={{ marginTop: 10 }}>
        <Glow color={G} size="0.85rem">🎯 HOW TO TRADE IT</Glow>
        <div style={{ marginTop: 8, display: "grid", gap: 8 }}>
      ...
      <Bx color={C} style={{ marginTop: 10 }}>
        <Glow color={C} size="0.8rem">💡 ANALOGY</Glow>
        <p style={{ color: "#999", fontSize: 13, lineHeight: 1.6, marginTop: 6 }}>A <strong style={{color:M}}>dog on a leash</strong> in a yard. The leash is 60% of how far the dog ran yesterday. Most of the time, the dog stays within range. If it pulls hard enough to snap the leash — something big is happening.</p>
      </Bx>
    </div>
  );
  ...
  const renderPDMid = () => (
    <div>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <Glow color="#AAAAAA" size="1.5rem">📏 PD MIDLINE</Glow>
        <p style={{ color: D, fontSize: 12, marginTop: 4 }}>The line that separates cheap from expensive.</p>
      </div>
      <Bx color={G}>
        <Glow color={G} size="0.85rem">WHAT IT IS</Glow>
        <p style={{ color: "#ccc", fontSize: 13, lineHeight: 1.7, marginTop: 8 }}>The <strong style={{color:Y}}>50% retracement of yesterday's range</strong>. One dotted line. Above it = <strong style={{color:R}}>Premium</strong> (price is expensive — look for shorts). Below it = <strong style={{color:G}}>Discount</strong> (price is cheap — look for longs). Institutions use this level constantly.</p>
      </Bx>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 12 }}>
        <Bx color={R}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 28 }}>📈</div>
            <Glow color={R} size="0.9rem">PREMIUM</Glow>
            <div style={{ fontSize: 11, color: "#aaa", marginTop: 6 }}>Above PD Mid</div>
            <div style={{ fontSize: 12, color: R, marginTop: 4, fontWeight: "bold" }}>Look for SHORTS</div>
            <div style={{ fontSize: 10, color: "#666", marginTop: 4 }}>Price is expensive. Sellers step in.</div>
          </div>
        </Bx>
        <Bx color={G}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 28 }}>📉</div>
            <Glow color={G} size="0.9rem">DISCOUNT</Glow>
            <div style={{ fontSize: 11, color: "#aaa", marginTop: 6 }}>Below PD Mid</div>
            <div style={{ fontSize: 12, color: G, marginTop: 4, fontWeight: "bold" }}>Look for LONGS</div>
            <div style={{ fontSize: 10, color: "#666", marginTop: 4 }}>Price is cheap. Buyers step in.</div>
          </div>
        </Bx>
      </div>
      <Bx color={Y} style={{ marginTop: 12 }}>
        <Glow color={Y} size="0.85rem">🎯 THE COMBO PLAY</Glow>
        <p style={{ color: "#ccc", fontSize: 13, lineHeight: 1.7, marginTop: 8 }}>When your script fires a LONG signal AND you're below PD Mid (in discount) = <strong style={{color:G}}>high conviction</strong>. A LONG signal in Premium? <strong style={{color:R}}>Lower conviction</strong> — you're buying expensive. PD Mid gives every signal a quality grade.</p>
      </Bx>
      <Bx color={C} style={{ marginTop: 10 }}>
        <Glow color={C} size="0.8rem">💡 ANALOGY</Glow>
        <p style={{ color: "#999", fontSize: 13, lineHeight: 1.6, marginTop: 6 }}>A <strong style={{color:Y}}>gas station price board</strong>. Above average = you feel ripped off (premium). Below average = great deal (discount). PD Mid is the line between "good deal" and "overpaying."</p>
      </Bx>
    </div>
  );
  ...
  const renderOpens = () => (
    <div>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <Glow color={Y} size="1.5rem">🔔 SESSION OPENS</Glow>
        <p style={{ color: D, fontSize: 12, marginTop: 4 }}>Where the big players started their day. Price loves to come back.</p>
      </div>
      <Bx color={Y}>
        <div style={{ display: "grid", gap: 10 }}>
          {[
            ...
            <div key={i} style={{ padding: "12px 14px", borderRadius: 8, background: `${o.c}06`, borderLeft: `3px solid ${o.c}40` }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 13, color: o.c, fontWeight: "bold" }}>{o.name}</span>
                <span style={{ fontSize: 10, color: D }}>{o.time}</span>
              </div>
              <div style={{ fontSize: 12, color: "#aaa", marginTop: 6, lineHeight: 1.6 }}>{o.d}</div>
            </div>
          ))}
        </div>
      </Bx>
      <Bx color={G} style={{ marginTop: 12 }}>
        <Glow color={G} size="0.85rem">🎯 HOW TO USE OPENS</Glow>
      ...
      <Bx color={C} style={{ marginTop: 10 }}>
        <Glow color={C} size="0.8rem">💡 ANALOGY</Glow>
        <p style={{ color: "#999", fontSize: 13, lineHeight: 1.6, marginTop: 6 }}>The <strong style={{color:Y}}>starting line of a race</strong>. No matter how far the runners go, they measure distance from where they started. Price loves to come back and touch the starting line.</p>
      </Bx>
    </div>
  );
  ...
  const renderHowToUse = () => (
    <div>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <Glow color={Y} size="1.5rem">📖 HOW TO USE AURA MAP</Glow>
        <p style={{ color: D, fontSize: 12, marginTop: 4 }}>The question checklist that makes every trade better.</p>
      </div>
      <Bx color={C}>
        <Glow color={C} size="0.85rem">BEFORE EVERY TRADE, ASK:</Glow>
        <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
          {[
            ...
            <div key={i} style={{ padding: "10px 14px", borderRadius: 8, background: `${item.c}06`, border: `1px solid ${item.c}15` }}>
              <div style={{ fontSize: 13, color: item.c, fontWeight: "bold" }}>{item.q}</div>
              <div style={{ fontSize: 12, color: "#aaa", marginTop: 4, lineHeight: 1.5 }}>{item.answer}</div>
            </div>
          ))}
        </div>
      </Bx>
      <Bx color={G} style={{ marginTop: 12 }}>
        <Glow color={G} size="0.85rem">🟢 LIVE EXAMPLE: MAP + ZONEWARS</Glow>
      ...
      <Bx color={R} style={{ marginTop: 12 }}>
        <Glow color={R} size="0.85rem">🔴 WHEN MAP SAYS "CAREFUL"</Glow>
        <div style={{ marginTop: 8, fontSize: 12, color: "#bbb", lineHeight: 1.8 }}>
  ...
  const renderThemes = () => (
    <div>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <Glow color={M} size="1.5rem">🎨 9 THEMES</Glow>
        <p style={{ color: D, fontSize: 12, marginTop: 4 }}>One click. Every color syncs. Pick your vibe.</p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
        {themes.map((t, i) => (
          <Bx key={i} color={t.c} style={{ textAlign: "center", padding: "14px 8px", cursor: "pointer" }}>
            <div onClick={() => setSelTheme(i)}>
              <div style={{ fontSize: 22 }}>{t.icon}</div>
              <div style={{ fontSize: 10, color: t.c, fontWeight: "bold", marginTop: 4 }}>{t.name}</div>
            </div>
          </Bx>
        ))}
      </div>
      <Bx color={M} style={{ marginTop: 12 }}>
        <Glow color={M} size="0.8rem">💡 ANALOGY</Glow>
        <p style={{ color: "#999", fontSize: 13, lineHeight: 1.6, marginTop: 6 }}>A <strong style={{color:M}}>DJ lighting board</strong> at a concert. One slider changes every light in the venue at once. Cyberpunk = neon rave. Matrix = hacker den. Gold Elite = VIP lounge. Same music, completely different vibe.</p>
      </Bx>
    </div>
  );
  ...
  const renderSetup = () => (
    <div>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        ...
        <p style={{ color: D, fontSize: 12, marginTop: 4 }}>Context on your chart in 60 seconds.</p>
      </div>
      {[
        { s: 1, t: "Add AURA MAP to Chart", d: "NQ1! or MNQ1! • Any timeframe • Apply AURA MAP overlay", c: G },
        ...
        { s: 5, t: "Layer Under Your Weapon", d: "Add ZoneWars, LSM, Midas Touch, or any weapon ON TOP of AURA MAP. Every signal now has context.", c: G },
      ].map((s, i) => (
      ...
      <Bx color={G} style={{ marginTop: 14 }}>
        ...
        <div style={{ marginTop: 10 }}>
          {[
            ...
            "When your weapon fires a signal → run the MAP checklist",
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: W, fontFamily: "'Segoe UI',sans-serif" }}>
      ...
      <div style={{ textAlign: "center", padding: "16px 0 24px", borderTop: `1px solid ${D}22` }}>
        <span style={{ fontSize: 10, color: D, fontFamily: "'Orbitron',sans-serif" }}>AURASZN × AURA MAP 📡 • VISUAL INTELLIGENCE OVERLAY • 2026</span>
      </div>
    </div>
  );
}
